...
- **Dev server:** `scripts/serve.ts` — Bun.serve() handles SSR (all routes), static files (`/app.js`, `/style.css`), and API stub (`/api/*`)
...
- When adding a new static file to `dist/`, a corresponding route must be added in `scripts/serve.ts`.
